...
switch (true) do
{
	...
	case (_veh isKindOf "Tank") :
	{
		switch(true) do
		{
			...
			case (damage _veh > 0.04) : {_price = 150;};
			case (damage _veh > 0.03) : {_price = 125;};
			default {_price = 10;};
		};
		
		switch (true) do
		{
			case (fuel _veh < 0.2) : {_price = _price + 120;};
			case (fuel _veh < 0.5) : {_price = _price + 115;};
			case (fuel _veh < 0.8) : {_price = _price + 110;};
			default { _price = _price + 5;};
		};
	};
};
