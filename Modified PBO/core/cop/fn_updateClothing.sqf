...
if(playerSide == independent && license_med_adac) then {
	switch(true) do {
		case (uniform player == "U_Competitor") : {
			player setObjectTextureGlobal [0,"textures\clothing\aa.jpg"];
		};
		case(backpack player == "B_Carryall_khk") then {
		(backpackContainer player) setObjectTextureGlobal [0, "textures\clothing\medic_backpack.jpg"];
		};
...
if(playerSide == civilian && license_civ_bountyh) then {
	switch(true) do {
	...
	if(backpack player == "B_Kitbag_cbr") then {
		(backpackContainer player) setObjectTextureGlobal [0, "textures\clothing\agentbackpack.jpg"];
	};
};
