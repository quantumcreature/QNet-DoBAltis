...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","mongrel","power","head","kart"] && playerSide != civilian) exitWith {hint localize "STR_Shop_NotaCiv"; closeDialog 0;};
...
//if((_this select 3) in ["bhc"] && !license_civ_bountyh) exitWith { hint "You have no license"; closeDialog 0;};
//if((_this select 3) in ["adac"] && !license_med_adac) exitWith { hint "You have no license"; closeDialog 0;};
//if((_this select 3) in ["donator"] && !license_civ_donator) exitWith { hint "You have no Access"; closeDialog 0;};
